...
const userAttributes = ["id", "username", "email", "role", "created_at", "updated_at"];

export const getUsers = async (req, res) => {
  try {
    const users = await User.findAll({ attributes: userAttributes, order: [["id", "DESC"]] });
...
export const updateUserRole = async (req, res) => {
  try {
    ...
    return res.json({ status: "success", message: "User role updated", data: user });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
};
